...
namespace Components.ObjectDatabase
{
    public abstract class ObjectDatabaseBase<TElement> : CriticalFinalizerObject, IDatabaseContext<TElement>
    {
        ...
        public ObjectDatabaseBase(string filename)
        {
        ...
        {
            Sync(Filename.ToLower(), action);
        }

        public TResult Sync<TResult>(Func<TResult> func)
        {
            return Sync(Filename.ToLower(), func);
        }
